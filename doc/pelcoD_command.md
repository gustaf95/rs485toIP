...
↔ EDIS ED-P 트래픽을 캡처해 복원했다. `SET`의 `pp`/`qq`는 VISCA 명령 코드/값과 **대체로** 같다 —
AWB(`pp`=`36` vs VISCA `35`)만 예외이므로, 새 파라미터를 추가할 때 `pp`를 VISCA 코드로 가정하지
말고 실제로 나가는 바이트를 캡처해서 확인해야 한다.
...
| **AWB Auto / Manual** | `C3 **36** 00` / `C3 **36** 05` | `8x 01 04 **35** 00/05 FF` | 🔬 (2026-08-12) **`pp`가 VISCA와 다른 유일한 사례** — 아래 참고 |
| **Back Light On / Off** | `C3 33 02` / `C3 33 03` | `8x 01 04 33 02/03 FF` | 🔬📖 (2026-08-08) |
| **R Gain Up / Down** | `C3 03 02` / `C3 03 03` | `8x 01 04 03 02/03 FF` | 🔬📖 (2026-08-12) 1:1 |
| **B Gain Up / Down** | `C3 04 02` / `C3 04 03` | `8x 01 04 04 02/03 FF` | 🔬📖 (2026-08-12) 1:1 |
| **Shutter Up / Down** | `C3 0A 02` / `C3 0A 03` | `8x 01 04 0A 02/03 FF` | 🔬📖 (2026-08-12) 1:1. Down(`03`)만 실측 |
| **BRIGHT Up / Down** | `C3 0E 02` / `C3 0E 03` | `8x 01 04 **0E** 02/03 FF` | 🔬📖 (2026-08-12) 1:1. `CAM_ExpComp`이지 `CAM_Bright`(`0D`)가 **아니다** |
| **One Push AF** | `C3 18 01` | `8x 01 04 38 04 FF`, **2초 뒤** `8x 01 04 38 03 FF` | 🔬 아래 참고 |

#### 절대 모드 vs 상대 조정

위 표는 성격이 다른 두 부류가 섞여 있다.

- **절대 모드** (Power, Iris, Focus, AWB, Back Light) — "이 값으로 설정하라". 멱등이라 `modeCache`에
  담고 `D3` 조회에 답할 수 있다.
- **상대 조정** (R Gain `03`, B Gain `04`, Shutter `0A`, ExpComp `0E`) — "한 칸 올려라/내려라".
  담을 상태가 없어 캐시하지 않고 확인 조회도 예약하지 않는다.

**상대 조정에서는 `qq`의 뜻이 다르다** — `02`가 On/Auto가 아니라 **Up**, `03`이 Off/Manual이 아니라
**Down**이다. VISCA 값 표(`00` Reset / `02` Up / `03` Down)를 그대로 따른다. Reset(`00`)은 컨트롤러에서
관측되지 않아 통과 목록에 없다.

반복 전송이 곧 "한 칸 더"이므로 **이 넷만 중복 억제(`VISCA_DUPLICATE_SUPPRESS_MS`)에서 제외된다.**
억제하면 200ms에 한 칸씩만 통과해 키를 눌러도 화면이 안 움직이는 것처럼 보인다. 이 문제는 게이트웨이
슬롯에만 생긴다 — 같은 버스의 실물 ED-P는 컨트롤러 패킷을 직접 받으므로 억제를 거치지 않는다.

각각 전제 조건이 있지만 게이트웨이는 강제하지 않고 그대로 흘린다 — R/B Gain은 AWB가 Manual일 때,
ExpComp Up/Down은 ExpComp가 On(`04 3E 02`)일 때 의미가 있다.

> **노출 관련 상대 조정은 `0x0A`~`0x0E`가 연속된 한 벌이다** — Shutter(`0A`), Iris(`0B`), Gain(`0C`),
> Bright(`0D`), ExpComp(`0E`). 실측된 건 `0A`와 `0E` 둘뿐이라 그 둘만 통과시킨다. 컨트롤러에 IRIS/GAIN
> 키가 따로 있다면 `0B`/`0C`도 나올 텐데, 실제 바이트를 잡기 전에는 넣지 않는다 —
> AWB(`36` ≠ VISCA `35`)에서 "`pp` = VISCA 코드"를 가정했다가 키가 통째로 먹지 않았던 전례가 있다.
> [`todo.md` 2.6절](todo.md) 참고.
>
> 통과 목록에 없는 `pp`는 `Unknown set parameter 0x..`로 로그에 남고 Unhandled Commands에 쌓인다.
> **키를 눌러도 아무 반응이 없으면 Debug Mode에서 이 줄부터 확인하는 게 가장 빠르다** — `[EDIS->VISCA]`
> 줄이 없다면 번역 전에 걸러진 것이므로 카메라까지 간 적이 없다.

#### BRIGHT 키는 `CAM_ExpComp`다

키 라벨만 보면 `CAM_Bright`(`0D`)로 짐작하기 쉽지만 실제로 나가는 바이트는 `0E`였다. 앞뒤도 맞는다 —
`CAM_Bright`는 AE 모드가 Bright(`04 39 0D`)일 때만 먹는데, 이 컨트롤러의 IRIS AUTO/MANUAL 키는 AE를
Full Auto(`00`)나 Manual(`03`)로만 보내므로 애초에 Bright 모드가 될 일이 없다. ExpComp는 자동 노출에
보정값을 얹는 방식이라 Full Auto에서 동작한다.

> ⚠️ **동작 조건이 Sony VISCA와 반대라 번역이 확정되지 않았다.** ED-P는 IRIS **Manual**에서 동작하는데
> `CAM_ExpComp`는 AE가 **Auto**일 때 동작하고 Manual에서는 무효다. 지금은 `04 0E`를 그대로 내보내지만,
> ED-P의 동작을 그대로 옮기려면 `CAM_Iris`(`04 0B`)여야 할 수도 있다. FoMaKo 실측으로 갈린다 —
> [`todo.md` 2.5절](todo.md) 참고.

#### 실행할 수 없는 SET에는 거절 응답 `0x66`이 온다

```
FF ADDR 00 66 00 01 CK
```

2026-08-12 해독. ED-P에서 IRIS를 바꿔가며 갈랐다 — Manual이면 밝기가 바뀌고 응답이 없고, Auto면
밝기가 안 바뀌고 `0x66`이 온다. 즉 `0x66`은 ACK가 아니라 VISCA `y0 60 41 FF`(Command Not Executable)에
해당하는 자리이고, **"`C3` SET에는 응답이 없다"는 규칙은 성공한 SET에 한해 그대로 유효하다.**
Up/Down이 같은 응답을 받는 것도 이걸로 설명된다 — 값이 아니라 거절 사유를 담으니 방향과 무관하다.

게이트웨이는 아직 이 거절을 합성하지 않는다. [`todo.md` 2.4절](todo.md), [3.4절](todo.md) 참고.

> **AWB의 `pp`는 VISCA 코드가 아니다 (2026-08-12 실측으로 정정).** 컨트롤러가 실제로 보내는
> 바이트는 `0x36`인데 VISCA `CAM_WB`는 `0x35`다:
>
> ```
...
> ```
>
> `qq`(값)는 VISCA 그대로다 — `00` Auto / `05` Manual이 VISCA `CAM_WB` 값 표와 정확히 같다.
> 어긋나는 건 명령 코드 한 바이트뿐이라, 게이트웨이가 `36` → `35`로 갈아끼워 내보낸다.
>
> 이전 표에는 `C3 35 pq`로 적혀 있었는데, 그건 실측이 아니라 "`pp` = VISCA 코드" 규칙에서
> 역산한 추정이었다(근거 칸도 "SET은 미관측, 응답에서 코드 확인"이었다). 실측으로 확인됐던 건
> `D3 19` **응답**에 실리는 WB 코드가 VISCA 값이라는 것뿐이고 요청 쪽 `pp`는 확인된 적이
> 없었다. **그래서 실제 컨트롤러의 AWB 키가 먹지 않았다** — `36`이 미검증 파라미터로 걸러져
> Unhandled Commands에만 쌓였다.
>
> One Push AF도 1:1이 아니지만 원인이 다르다 — 그쪽은 `pp`(`18`)가 진짜 VISCA 코드인데
> FoMaKo가 구현하지 않은 **카메라 쪽** 문제고, AWB는 `pp` 자체가 VISCA가 아닌 **컨트롤러 쪽**
> 문제다.
...
실물 ED-P와 충돌 방지), 카메라가 자고 있으면 조회에 침묵한다 — **단 전원 조회(`D3 04 00`)만은
스탠바이 중에도 답한다**(아래 참고).
...
> **스탠바이 중 전원 조회는 답한다 (2026-08-12 실측으로 정정).** 이전엔 "카메라가 켜져 있지 않으면
> 어떤 조회에도 침묵한다"였고, 근거는 "실물 ED-P도 그렇게 한다"였다. 이건 실측이 아닌 추정이었고
> **틀렸다** — 잠든 3번 ED-P가 전원 조회에 자기 상태를 정직하게 돌려준다:
>
> ```
> FF 03 00 D3 04 00 DA  ->  FF 03 00 D7 00 03 DD      값 03 = Standby
> ```
>
> 하필 전원 조회가 "너 켜져 있냐"는 질문 그 자체라, 여기에 침묵하면 컨트롤러는 **"자는 중"과
> "그 주소에 아무것도 없음"을 구분할 수 없다.** 답할 값을 캐시에 이미 갖고 있으므로 알려주지 않을
> 이유가 없다.
>
> 나머지 조회(`D3 19` 모드 상태, `D3 04 33` BLC)는 스탠바이 중 침묵을 유지한다 — 잠든 동안 그
> 값들은 의미가 없고, 실물이 그때 무엇을 답하는지는 아직 실측하지 않았다.
>
> 캐시의 전원 값은 두 경로로 정확하게 유지된다: VISCA `CAM_PowerInq`(`09 04 00`) 주기 조회와,
> 컨트롤러의 `C3 00 03`이 지나갈 때의 즉시 반영. 후자 덕분에 카메라가 잠든 채 `CAM_PowerInq`에
> 답하지 않더라도 값은 맞는다.
