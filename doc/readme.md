...
### UART 예산 — 두 보드 모두 RS485 2포트까지

지금 코드는 RS485 포트를 **하나만** 연다. 늘릴 여지가 얼마나 있는지는 이렇게 된다:

| | 총 UART | 콘솔이 먹는 것 | RS485에 남는 것 |
| --- | --- | --- | --- |
| ESP32 클래식 | 3 | UART0 (GPIO1/3, USB 브리지 칩에 하드와이어) | **2** |
| ESP32-C3 | 2 | 없음 (네이티브 USB CDC) | **2** |

C3가 UART 하나 적은 것을 **콘솔이 UART를 안 쓴다는 점**이 상쇄한다. 양쪽 다 여유는 0이라,
3포트 이상이 필요해지면 C3로는 안 되고 ESP32-S3로 가야 한다.

이 셈은 전적으로 `-DARDUINO_USB_CDC_ON_BOOT=1`에 의존한다 — 그 플래그가 빠지면 C3의
`Serial`이 UART0로 돌아가서 RS485는 1포트뿐이다. 두 번째 포트를 붙일 때 걸리는 것들은
[`todo.md` 3.6절](todo.md)에 정리해뒀다.
...
- **AP에서도 허용한다.** 제어 패널의 PTZ 조작은 `WebControl::controlAllowed()`가 AP를
  막지만(`todo.md` 4.6절의 이유), 펌웨어 업로드는 막지 않는다 — 운영자의 결정이다. 설정
...
**`led.active_low`도 같이 되돌린다.** 핀 번호는 아니지만 그 핀에 무엇이 어떻게 물렸는지를
말하는 값이라(3.4절), 핀을 거부했으면 극성도 같이 거부해야 한다. 이게 없으면 클래식 →
C3에서 정확히 틀린다:

| | 클래식 기본값 | C3 Super Mini 기본값 |
| --- | --- | --- |
| `led.pin` | 13 (외부 LED) | 8 (온보드 파란 LED) |
| `led.active_low` | 0 (액티브 하이) | 1 (액티브 로우) |

핀 세트가 거부되어 `led.pin`은 C3의 8로 되돌아가는데 극성만 파일의 `0`이 남으면, 온보드
LED가 꺼져야 할 때 켜지고 켜져야 할 때 꺼진다. 배선을 아무리 봐도 원인이 안 나오는 종류의
증상이다.

파일에 `led.pin` 없이 `led.active_low`만 있으면 이 되돌리기에 걸리지 않는다 — 그건 지금
핀의 극성만 고치겠다는 뜻이라 그대로 적용된다.

**다른 보드에서 받은 파일이 여기 걸린다.** 클래식 ESP32와 ESP32-C3는 쓸 수 있는 GPIO가
거의 겹치지 않아서(3.1절), 어느 방향이든 핀 세트는 통째로 거부되고 Wi-Fi·라우팅·프로토콜 등
나머지는 그대로 복원된다. 예를 들어 클래식의 기본 핀(RX 25 / TX 26 / DE-RE 27)은 C3의 GPIO
상한(21)을 넘어 첫 검사에서 걸리고, C3의 기본 핀(4/5/6)은 클래식에서 6번이 내장 플래시(6~11)라
걸린다. 펌웨어 이미지(13.4.1절)와 달리 **보드가 다르다고 파일 전체를 거부하지는 않는다** —
잘못 올리면 벽돌이 되는 펌웨어와 달리, 설정은 안 맞는 부분만 빼고 받는 쪽이 쓸모 있기
때문이다.

남는 위험 하나는 **양쪽 보드에서 모두 유효한 핀 조합**이다. 그때는 검사를 통과해서 그대로
들어가는데, 파일이 기록한 것은 원래 보드의 배선이므로 새 보드의 실제 결선과 다를 수 있다.
보드를 바꿔 복원한 뒤에는 RS485 화면의 핀 값을 한 번 확인하는 편이 안전하다.
